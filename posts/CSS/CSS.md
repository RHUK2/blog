...
- [CSS](#css)
  ...
  - [flex와 grid, media query](#flex와-grid-media-query)
...
기본적으로 `width: 100%`로 설정되어 있기 때문에 따로 설정할 필요 없음. 가로 수평 스크롤을 피하기 위해서는 `width: 100vw`는 지양하자. 초기값을 `100%` 이하로 잡을 때는 설정 필요. 가장 부모에게 고정 너비를 부여하고 밑에 자식들은 퍼센트로 관리하기.
...
## flex와 grid, media query

반응형 작업을 위해서는 `flex`, `grid`, `media query`를 필수로 사용해야한다.
...
`max-content`는 안에 내용물이 가질 수 있는 최대 길이를 의미한다. 나 같은 경우, 버튼의 태그에 `width: max-content`를 적용해 텍스트가 줄어들지 않고 고정되도록 사용했다. 또는 `display: grid`에서 열 너비를 정할 때에도 사용했다. 반응형으로 작성하면서 내용물의 너비로 고정시키고 싶은 경우 많이 사용했다.
...
자동완성의 스타일링 `!important`로 브라우저 정의가 되어있어 `box-shadow로` 배경색을 제거해주는 트릭을 사용한다.
