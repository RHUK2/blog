...
```text
[사용자] → [클라이언트 앱] → [권한 부여 서버] → [자원 서버]
```

Step 1: 권한 부여 요청 (클라이언트 → 권한 부여 서버)

클라이언트는 사용자를 권한 부여 서버의 인증 페이지로 리다이렉트한다.
...
- `response_type=code`: Authorization Code를 응답으로 요청함
- `state`: CSRF 방지용 무작위 값. 응답에서 동일한 값이 돌아와야 함

Step 2: 사용자 인증 및 동의 (권한 부여 서버)

권한 부여 서버는 사용자에게 로그인 화면과 권한 동의 화면을 제공한다. 사용자가 동의하면 다음 단계로 진행한다.

Step 3: Authorization Code 발급 (권한 부여 서버 → 클라이언트)

권한 부여 서버는 클라이언트의 `redirect_uri`로 Authorization Code를 전달한다.
...
클라이언트는 응답의 `state` 값이 Step 1에서 보낸 값과 일치하는지 반드시 검증해야 한다.

Step 4: Access Token 교환 (클라이언트 → 권한 부여 서버)

클라이언트 서버는 Authorization Code를 Access Token으로 교환하기 위해 권한 부여 서버에 직접 요청을 보낸다. 이 요청은 브라우저가 아닌 서버에서 수행된다.
...
Step 5: Access Token 응답 (권한 부여 서버 → 클라이언트)
...
Step 6: 자원 접근 (클라이언트 → 자원 서버)

클라이언트는 발급받은 Access Token을 `Authorization` 헤더에 담아 자원 서버에 요청한다.
...
Step 2: 권한 부여 요청에 `code_challenge` 포함
...
Step 3~4: Authorization Code 수신 후 토큰 교환 요청에 `code_verifier` 포함
...
권한 부여 서버는 `code_verifier`를 해시하여 Step 2에서 받은 `code_challenge`와 일치하는지 검증한다. 일치하지 않으면 토큰 발급을 거부한다.
...
Step 1: Access Token 요청 (클라이언트 → 권한 부여 서버)
...
Refresh Token은 발급되지 않는다. 토큰 만료 시 동일한 요청으로 새 토큰을 획득한다.
...
Step 1: Access Token 만료 감지

자원 서버가 `401 Unauthorized`를 반환하면 클라이언트는 토큰 갱신을 시도한다.

Step 2: 새 Access Token 요청 (클라이언트 → 권한 부여 서버)
