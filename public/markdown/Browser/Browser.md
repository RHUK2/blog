...
![browser_rendering](images/browser_rendering.png)
...
![browser_url_input](images/browser_url_input.jpg)
...
![compare_rendering](images/compare_rendering.png)
...
위 문장은 개념적으로 맞지만, 좀 더 명확하게 설명할 수 있다. 키보드 입력 시 키에 맞는 유니코드를 프로그램에 전달하고, 이를 UTF-8로 인코딩하여 메모리에 저장하며, 화면에 출력할 때는 UTF-8로 디코딩하여 출력하는 과정은 아래와 같다:

1. **키보드 입력 시 유니코드 전달**:

   - 키보드에서 키를 누르면 운영 체제와 입력 처리 시스템이 해당 키에 맞는 유니코드 코드 포인트를 생성하여 프로그램에 전달한다.

2. **UTF-8로 인코딩하여 메모리에 저장**:

   - 프로그램은 이 유니코드 코드 포인트를 UTF-8 인코딩 방식으로 변환하여 메모리에 저장한다. UTF-8은 가변 길이 인코딩 방식으로, 1바이트에서 4바이트까지 사용하여 유니코드 문자를 표현한다.

3. **화면에 출력할 때 UTF-8로 디코딩**:
   - 화면에 문자를 출력할 때는 메모리에 저장된 UTF-8 인코딩 데이터를 읽어와 UTF-8 디코딩을 통해 원래의 유니코드 문자로 변환한다.
   - 변환된 유니코드 문자는 화면에 출력되기 위해 렌더링 엔진에 의해 처리된다.

이 과정을 통해 키보드 입력이 유니코드로 프로그램에 전달되고, UTF-8로 인코딩 및 디코딩되어 메모리에 저장되고 화면에 출력된다.
...
1. **키보드 입력**:
   ...
   - 사용자가 키보드에서 'A' 키를 누른다.
   - 운영 체제는 'A'의 유니코드 코드 포인트인 U+0041을 프로그램에 전달한다.

2. **메모리에 저장**:

   - 프로그램은 U+0041을 UTF-8로 인코딩한다.
   - 'A'는 UTF-8에서 0x41로 인코딩되며, 이는 1바이트로 표현된다.
   - 이 1바이트 데이터(0x41)를 메모리에 저장한다.

3. **화면 출력**:
   - 메모리에서 0x41을 읽어온다.
   - 이 값을 UTF-8 디코딩하여 U+0041(유니코드 문자 'A')로 변환한다.
   - 변환된 유니코드 문자는 화면에 출력된다.

따라서, 원래 문장은 개념적으로 옳으며, 키보드 입력 시 키에 맞는 유니코드를 프로그램에 전달하고, 이를 UTF-8로 인코딩하여 메모리에 저장하고, 화면에 출력할 때 UTF-8로 디코딩하여 출력하는 과정이다.
...
![cache_size](images/cache_size.png)
...
![payload_multipart1](images/payload_multipart1.png)

![payload_multipart2](images/payload_multipart2.png)
...
![payload_json1](images/payload_json1.png)

바이너리 데이터를 Base64 형식으로 인코딩해서 보내는 경우는 아래와 같다.

![payload_json2](images/payload_json2.png)
...
![payload_octet_stream](images/payload_octet_stream.png)

### application/x-www-form-urlencoded

HTML 폼 방식으로 HTTP 요청을 할 때 기본값으로 설정되며, HTTP 요청의 body 값으로 데이터를 URL 인코딩하여 보낼 때 사용한다.

![payload_form_urlencoded1](images/payload_form_urlencoded1.png)

![payload_form_urlencoded2](images/payload_form_urlencoded2.png)
...
![initial_connection](images/initial_connection.png)
...
![preview_image](images/preview_image.png)
