...
서버 사이드 리다이렉트(Server-side Redirect)는 서버가 클라이언트에게 특정 URL로 이동하라는 응답을 보내는 메커니즘이다. 주로 `3xx` 상태 코드와 `Location` 헤더를 사용하여 구현한다.

- 서버 사이드 렌더링(SSR) 및 전통적 웹:
  - 브라우저가 서버의 리다이렉트 응답을 받으면 `Location` 헤더에 명시된 URL로 자동으로 페이지를 이동시킴.
  - 페이지 전체가 새로고침되며 새로운 문서를 서버에 요청함.
- 싱글 페이지 애플리케이션(SPA) 및 AJAX 요청:
  - `fetch`나 `axios` 등을 통해 비동기 API를 호출할 때는 브라우저가 서버의 리다이렉트 응답을 자동으로 따라가지 않음.
  - 서버는 리다이렉트 대상 URL을 응답 본문에 담아 전달하고, 클라이언트 코드(JavaScript)가 이를 해석하여 `History API` 등을 통해 직접 라우팅을 처리해야 함.

요약 비교:

| 환경            | 동작 방식                           | 결과                                               |
| :-------------- | :---------------------------------- | :------------------------------------------------- |
| SSR / 전통적 웹 | 서버 리다이렉트 응답 (`302`, `301`) | 브라우저가 자동으로 URL 이동 및 페이지 로드        |
| SPA + AJAX      | 서버 응답 수신 후 클라이언트 처리   | 스크립트가 응답을 확인하여 직접 소프트 라우팅 수행 |
