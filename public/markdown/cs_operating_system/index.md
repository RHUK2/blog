...
- 단점: 세그먼트 크기가 가변적이므로 빈 공간이 작은 조각으로 흩어지는 외부 단편화(External Fragmentation)가 발생함. 현대 OS가 고정 크기 페이징을 주로 채택하는 이유임

### 페이지 폴트 (Page Fault)

- 프로세스가 접근하려는 페이지가 현재 물리 메모리에 없는 경우 발생함
- 운영체제는 디스크에서 해당 페이지를 찾아 메모리에 로드함
- 물리 메모리가 가득 찬 경우 기존 페이지를 제거해야 하며, 이때 페이지 교체 알고리즘이 적용됨
  - FIFO: 가장 먼저 적재된 페이지를 제거함
  - LRU (Least Recently Used): 가장 오랫동안 사용되지 않은 페이지를 제거함. 참조 지역성을 반영하여 실무에서 널리 사용됨
  - Clock: LRU의 근사 알고리즘으로, 참조 비트를 순환 검사하여 제거 대상을 선택함
