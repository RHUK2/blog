...
서비스 이름으로 폴더를 구분하고, 해당 폴더 안에 깃 설정 파일에 사용자 이름과 이메일을 작성하여 설정을 완료한다.
...
저장 경로를 따로 설정해주지 않았다면, `~/.ssh` 위치에 저장이 된다. 그 중에서 `*.pub` 형식의 파일이 공개키(public key)이다. 나머지 파일이 개인키(private key)이다. 공개키의 내용을 복사해서 github ssh 설정에 붙여넣기 해준다. 이후 ssh 접속이 가능해진다.
...
git merge --abort # 현재 진행 중인 병합을 중단하고 이전 상태로 돌아간다.
```

### git rebase

한 브랜치의 커밋 내역을 다른 브랜치의 가장 최신 커밋 내역 위에 올려놓는 방식으로 병합한다.
