...
XSS와 CSRF의 차이는 단순히 스크립트를 실행하느냐, 혹은 `img`나 `form` 태그를 통해 요청을 보내느냐의 차이에 그치지 않습니다. 두 공격의 차이는 공격의 본질과 목적에서 비롯됩니다. 자세히 설명하면 다음과 같습니다.
...
- CSRF 방어:
  - CSRF 토큰을 사용하여 서버가 요청이 신뢰할 수 있는 출처에서 왔는지 확인합니다.
  - SameSite 쿠키 속성을 설정하여 크로스 사이트 요청 시 쿠키가 전송되지 않도록 합니다.
...
따라서, XSS와 CSRF의 차이는 단순히 "스크립트를 실행하느냐"와 "태그를 통해 요청을 보내느냐"에 그치는 것이 아니라, 공격자가 목표로 하는 것과 공격 방법에서의 근본적인 차이에 있습니다.
...
1. CORS (Cross-Origin Resource Sharing) 설정 취약점
...
- 정의: 한 출처에서 다른 출처의 리소스를 페이지에 포함(embed)하는 작업을 의미합니다.
- 예시:
