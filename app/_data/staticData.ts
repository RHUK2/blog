    ...
    projectList: [
      ...
      },
      {
        ...
        experienceList: [
          ...
          { behavior: '반응형 개발', result: '모바일 화면 대응을 통한 사용자 경험 향상' },
          { behavior: 'Axios Instance를 Context API로 공유', result: '' },
          {
            behavior:
              '커스텀 에러를 이용하여 API 요청에 대한 예외 처리를 구분지어 관리, 처리되지 않은 에러는 Sentry로 전송',
            result: '',
          },
          { behavior: '프로젝트 초기 환경 설정, NPM Workspace를 이용한 Monorepo 적용, 라우터 구조 설계', result: '' },
          { behavior: '디자인 화면과 기능을 바탕으로 백엔드와 협업하여 초기 API 문서 작성', result: '' },
          { behavior: 'Next.js의 Middleware를 이용한 인증 • 권한 처리', result: '' },
          {
            behavior: 'Node.js 환경에서 PDFKit을 이용해 다면진단 보고서 구현',
            result: '',
          },
          { behavior: 'Next-Translate를 이용한 다국어 적용', result: '' },
          { behavior: 'Cloudflare Turnstile 적용', result: '' },
        ],
      },
      {
        ...
        experienceList: [
          { behavior: 'JQuery로 되어있던 서비스를 React로 리뉴얼', result: '유지보수와 추가 기능 개발이 용이해짐' },
          {
          ...
          { behavior: '반응형 개발', result: '모바일 화면 대응을 통한 사용자 경험 향상' },
          { behavior: '인사담당자, 협업부서, 헤드헌터, 어드민별 권한을 구분하여 접근 제어', result: '' },
          { behavior: 'React-Intl을 이용한 다국어 적용', result: '' },
        ],
      },
      {
        ...
        experienceList: [
          { behavior: 'Typescript 도입', result: '타입 안정성을 통한 예상치 못한 버그 발생 방지' },
          {
      ...
      },
      {
        ...
        experienceList: [
          { behavior: '토큰 만료 시 토큰 재발급 후 이전 실패된 요청에 대해 재요청 하는 로직 적용', result: '' },
          { behavior: 'CORS 에러 해결', result: '' },
          { behavior: 'Google reCaptcha 적용', result: '' },
        ],
      },
    ],
  },
